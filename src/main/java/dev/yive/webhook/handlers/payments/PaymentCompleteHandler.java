...
public class PaymentCompleteHandler implements Handler {
    @Override
    public void handle(@NotNull Context ctx) throws Exception {
        if (!Main.config.getPayments().getComplete().isEnabled()) return;
        ...
        embed.setColor(Math.round(Math.max(0, Math.max(0, paidPrice - giftCardsPrice)) * 100.0) / 100.0 > 0 ? Color.GREEN.getRGB() : Color.GRAY.getRGB());
