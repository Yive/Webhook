...
public class PaymentDeclinedHandler implements Handler {
    @Override
    public void handle(@NotNull Context ctx) throws Exception {
        if (!Main.config.getPayments().getDeclined().isEnabled()) return;
        ...
        Embed embed = DiscordUtils.createEmbed("Payment Denied", payment, subject, revenue);

        embed.setColor(Math.round(Math.max(0, revenue) * 100.0) / 100.0 > 0 ? DiscordUtils.convertColour(255, 200, 0) : DiscordUtils.convertColour(128, 128, 128));
