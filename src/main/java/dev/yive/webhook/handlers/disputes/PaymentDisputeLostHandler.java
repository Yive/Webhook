...
public class PaymentDisputeLostHandler implements Handler {
    @Override
    public void handle(@NotNull Context ctx) throws Exception {
        if (!Main.config.getDisputes().getLost().isEnabled()) return;
        ...
        String url = Main.config.getDisputes().getLost().getDiscord().getUrl();
        Embed embed = DiscordUtils.createEmbed("Chargeback Lost", payment, subject);
        double paidPrice = 0;
        ...
        embed.setColor(Math.round(Math.max(0, Math.max(0, paidPrice - giftCardsPrice)) * 100.0) / 100.0 > 0 ? DiscordUtils.convertColour(255, 0, 0) : DiscordUtils.convertColour(128, 128, 128));
