...
public class PaymentDisputeOpenedHandler implements Handler {
    @Override
    public void handle(@NotNull Context ctx) throws Exception {
        if (!Main.config.getDisputes().getOpened().isEnabled()) return;
        ...
        String url = Main.config.getDisputes().getOpened().getDiscord().getUrl();
        Embed embed = DiscordUtils.createEmbed("Chargeback Opened", payment, subject);
        double paidPrice = 0;
        ...
        embed.setColor(Math.round(Math.max(0, Math.max(0, paidPrice - giftCardsPrice)) * 100.0) / 100.0 > 0 ? DiscordUtils.convertColour(255, 175, 175) : DiscordUtils.convertColour(128, 128, 128));
