...
public class PaymentDisputeClosedHandler implements Handler {
    ...
    public void handle(@NotNull Context ctx) throws Exception {
        ...
        Embed embed = DiscordUtils.createEmbed("Chargeback Closed", payment, subject, revenue);

        embed.setColor(Math.round(Math.max(0, revenue) * 100.0) / 100.0 > 0 ? DiscordUtils.convertColour(0, 128, 0) : DiscordUtils.convertColour(128, 128, 128));
